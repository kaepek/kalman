...
def Kalman_Filter_1D_calculate_diff_theta(self, last_theta, current_theta):
    if self.max_theta_step == False:
        return (current_theta - last_theta)
    else:
...
def Kalman_Filter_1D_estimate_state_vector_eular_and_kalman(self, time_or_dt, s):
    ...
    if current_idx == -1:
        ...
        current_time = None
        dt = None
        if self.time_is_relative == False:
...
class KalmanJerk1D():
    ...
    step = Kalman_Filter_1D_estimate_state_vector_eular_and_kalman
    get_kalman_vector = Kalman_Filter_1D_get_kalman_vector
    get_eular_vector = Kalman_Filter_1D_get_eular_vector
